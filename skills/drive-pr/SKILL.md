...
1. **CI status** — `gh pr checks <PR>` plus `gh run list --branch <branch>` to catch failing runs that `pr checks` deduplicates behind a passing rerun. ALSO confirm the latest commit SHA actually has a run for every workflow you expect: `gh api '/repos/OWNER/REPO/actions/runs?head_sha=<sha>' --jq '.workflow_runs[].name'`. GitHub's push-trigger sometimes silently misses (we've seen langwatch-app-ci skip 2 consecutive pushes while pr-conventions fires fine). If a required workflow is missing for the current SHA, dispatch it manually: `gh workflow run <name>.yml --ref <branch>` and confirm with the same query.
